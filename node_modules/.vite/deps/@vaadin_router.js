...
//#endregion
//#region node_modules/@vaadin/router/dist/resolver/utils.js
...
//#endregion
//#region node_modules/@vaadin/router/dist/resolver/matchPath.js
...
function matchPath(routePath, path, exact = false, parentKeys = [], parentParams) {
	...
	for (let i = 1; i < m.length; i++) {
		...
		const prop = key.name;
		const value = m[i];
		if (value !== void 0 || !Object.prototype.hasOwnProperty.call(params, prop)) if (key.modifier === "+" || key.modifier === "*") params[prop] = value ? value.split(/[/?#]/u).map(decodeParam) : [];
...
//#endregion
//#region node_modules/@vaadin/router/dist/resolver/matchRoute.js
...
//#endregion
//#region node_modules/@vaadin/router/dist/resolver/resolveRoute.js
function resolveRoute(context) {
	if (isFunction(context.route.action)) return context.route.action(context);
}
//#endregion
//#region node_modules/@vaadin/router/dist/resolver/resolver.js
...
var Resolver = class {
	...
	*/
	async resolve(pathnameOrContext) {
		...
		const match = matchRoute_default(this.#root, this.__normalizePathname(context.pathname) ?? context.pathname, !!this.baseUrl);
		...
		async function next(resume = false, parent = matches?.value?.route, prevResult) {
			...
			if (matches.done) throw getNotFoundError(context);
			currentContext = {
				...context,
		...
		try {
			...
			const _error = error instanceof NotFoundError ? error : new ResolutionError(currentContext, {
				code: 500,
				cause: error
			});
...
//#endregion
//#region node_modules/@vaadin/router/dist/resolver/generateUrls.js
function cacheRoutes(routesByName, route, routes, cacheKeyProvider) {
	const name = route.name ?? cacheKeyProvider?.(route);
	if (name) if (routesByName.has(name)) routesByName.get(name)?.push(route);
	else routesByName.set(name, [route]);
...
function generateUrls(resolver, options = {}) {
	if (!(resolver instanceof resolver_default)) throw new TypeError("An instance of Resolver is expected");
	...
	return (routeName, params) => {
		...
		if (!cached) {
			...
			for (const item of tokens) if (!isString(item)) keys[item.name] = true;
			cached = {
				keys,
				tokens
			};
		...
		let url = tokensToFunction(cached.tokens, {
...
//#endregion
//#region node_modules/@vaadin/vaadin-development-mode-detector/vaadin-development-mode-detector.js
...
function isFlowProductionMode() {
	if (FlowClients) {
		if (Object.keys(FlowClients).map((key) => FlowClients[key]).filter((client) => client.productionMode).length > 0) return true;
	}
	return false;
}
...
if (window.Vaadin.developmentMode === void 0) window.Vaadin.developmentMode = isDevelopmentMode();
//#endregion
//#region node_modules/@vaadin/vaadin-usage-statistics/vaadin-usage-statistics-collect.js
...
//#endregion
//#region node_modules/@vaadin/router/dist/router-meta.js
...
usageStatistics();
//#endregion
//#region node_modules/@vaadin/router/dist/transitions/animate.js
...
//#endregion
//#region node_modules/@vaadin/router/dist/utils.js
function ensureRoute(route) {
	...
	if (!isFunction(route.action) && !Array.isArray(route.children) && !isFunction(route.children) && !isString(route.component) && !isString(route.redirect)) throw new Error(log(`Expected route config "${route.path}" to include either "component, redirect" or "action" function but none found.`));
	if (route.redirect) ["bundle", "component"].forEach((overriddenProp) => {
		if (overriddenProp in route) console.warn(log(`Route config "${String(route.path)}" has both "redirect" and "${overriddenProp}" properties, and "redirect" will always override the latter. Did you mean to only use "${overriddenProp}"?`));
	});
}
...
function createLocation({ chain = [], hash = "", params = {}, pathname = "", redirectFrom, resolver, search = "" }, route) {
	...
	return {
		...
		getUrl: (userParams = {}) => resolver ? getPathnameForRouter(compile(getRoutePath(chain))({
			...params,
			...userParams
		}), resolver) : "",
		hash,
...
function amend(fn, obj, ...args) {
	return (result) => {
		if (result && isObject(result) && ("cancel" in result || "redirect" in result)) return result;
		return maybeCall(obj?.[fn], obj, ...args);
	};
}
function processNewChildren(newChildren, route) {
	if (!Array.isArray(newChildren) && !isObject(newChildren)) throw new Error(log(`Incorrect "children" value for the route ${String(route.path)}: expected array or object, but got ${String(newChildren)}`));
...
//#endregion
//#region node_modules/@vaadin/router/dist/triggers/click.js
function getAnchorOrigin(anchor) {
	const { port, protocol } = anchor;
	return `${protocol}//${protocol === "http:" && port === "80" || protocol === "https:" && port === "443" ? anchor.hostname : anchor.host}`;
}
...
function vaadinRouterGlobalClickHandler(event) {
	...
	if ((anchor.origin || getAnchorOrigin(anchor)) !== window.location.origin) return;
...
//#endregion
//#region node_modules/@vaadin/router/dist/triggers/popstate.js
...
//#endregion
//#region node_modules/@vaadin/router/dist/triggers/navigation.js
var triggers = [];
...
//#endregion
//#region node_modules/@vaadin/router/dist/router.js
...
var Router = class extends resolver_default {
	/**
	...
	*/
	...
	async #resolveRoute(context) {
		...
		if (isFunction(route.children)) {
			...
			if (!isFunction(route.children)) ({children} = route);
		...
		return await Promise.resolve().then(async () => {
			if (this.#isLatestRender(context)) return await maybeCall(route.action, route, context, commands);
		}).then((result) => {
			if (result != null && (typeof result === "object" || typeof result === "symbol")) {
				if (result instanceof HTMLElement || result === notFoundResult || isObject(result) && "redirect" in result) return result;
			}
	...
	*/
	...
	async #fullyResolveChain(topOfTheChainContextBeforeRedirects, contextBeforeRedirects = topOfTheChainContextBeforeRedirects) {
		...
		const topOfTheChainContextAfterRedirects = contextAfterRedirects !== contextBeforeRedirects ? contextAfterRedirects : topOfTheChainContextBeforeRedirects;
		const isFound = getPathnameForRouter(getMatchedPath(contextAfterRedirects.chain ?? []), this) === contextAfterRedirects.pathname;
	...
	async #findComponentContextAfterAllRedirects(context) {
		...
		throw result instanceof Error ? result : new Error(log(`Invalid route resolution result for path "${context.pathname}". Expected redirect object or HTML element, but got: "${logValue(result)}". Double check the action return value for the route.`));
	}
	...
	async #runOnBeforeCallbacks(newContext) {
		...
		if (previousChain.length) {
			for (let i = 0; i < Math.min(previousChain.length, newChain.length); newContext.__divergedChainIndex++, i++) if (previousChain[i].route !== newChain[i].route || previousChain[i].path !== newChain[i].path && previousChain[i].element !== newChain[i].element || !this.#isReusableElement(previousChain[i].element, newChain[i].element)) break;
			newContext.__skipAttach = newChain.length === previousChain.length && newContext.__divergedChainIndex === newChain.length && this.#isReusableElement(newContext.result, previousContext.result);
			if (newContext.__skipAttach) {
				for (let i = newChain.length - 1; i >= 0; i--) callbacks = this.#runOnBeforeLeaveCallbacks(callbacks, newContext, { prevent }, previousChain[i]);
			...
			} else for (let i = previousChain.length - 1; i >= newContext.__divergedChainIndex; i--) callbacks = this.#runOnBeforeLeaveCallbacks(callbacks, newContext, { prevent }, previousChain[i]);
		}
		if (!newContext.__skipAttach) for (let i = 0; i < newChain.length; i++) if (i < newContext.__divergedChainIndex) {
	...
	async #runOnBeforeLeaveCallbacks(callbacks, newContext, commands, chainElement) {
		...
		if (this.#isLatestRender(newContext)) result = amend("onBeforeLeave", chainElement.element, location, commands, this)(result);
		if (!(isObject(result) && "redirect" in result)) return result;
	}
	async #runOnBeforeEnterCallbacks(callbacks, newContext, commands, chainElement) {
		const location = createLocation(newContext, chainElement.route);
		const result = await callbacks;
		if (this.#isLatestRender(newContext)) return amend("onBeforeEnter", chainElement.element, location, commands, this)(result);
	}
	...
	async #redirect(redirectData, counter = 0, renderId = 0) {
		...
		if (!(outlet instanceof Element || outlet instanceof DocumentFragment)) throw new TypeError(log(`Expected router outlet to be a valid DOM Element | DocumentFragment (but got ${outlet})`));
	}
	...
	#copyUnchangedElements(context, previousContext) {
		...
		for (let i = 0; i < (context.__divergedChainIndex ?? 0); i++) {
			...
			if (unchangedElement) if (unchangedElement.parentNode === deepestCommonParent) {
	...
	#addAppearingContent(context, previousContext) {
		...
		this.#disappearingContent = Array.from(deepestCommonParent?.children ?? []).filter((e) => this.#addedByRouter.has(e) && e !== context.result);
	...
	*/
	urlForName(name, params) {
		if (!this.#urlForName) this.#urlForName = generateUrls_default(this, { cacheKeyProvider(route) {
	...
	*/
	urlForPath(path, params) {
		return getPathnameForRouter(compile(path)(params ?? void 0), this);
	}
	/**
...
//#endregion
export { DEFAULT_TRIGGERS, Router, amend, copyContextWithoutNext, createLocation, createRedirect, getMatchedPath, getPathnameForRouter, maybeCall, processNewChildren, renderElement };
